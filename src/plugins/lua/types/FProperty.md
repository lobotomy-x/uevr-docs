...
These should actually almost always be named "ReturnValue" but you can test to be sure.
...
Retrieve the bitmask for all Property flags on the FProperty object.
Unlike [UEVR_UFunctions](UFunction.md) there is no method to set or unset property flags but you can check for flags using the list below

ex:
```lua
for name, flag in pairs(all_property_flags) do
  if property:get_property_flags() & flag ~= 0 then
    print(name)
  end
end
```
